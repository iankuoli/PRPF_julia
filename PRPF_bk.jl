...
function pred_Preference(usr_idx::Array{Int64}, itm_idx::Array{Int64}, u::Int64,
  ...
  if model_type == "PairPRPF"
    ...
    ret[1,js] = @fastmath user_preference_train_pw(vec_subPrior_X_u, vec_subPredict_X_u, vec_subMatX_u, C, alpha, delta)

  elseif model_type == "LuceExpPRPF"
    # Prediction w.r.t user $u$ by luce-based list-wise LTR with exponential transformation
    ret[1,js] = @fastmath user_preference_train_luce(vec_subPrior_X_u, vec_subPredict_X_u, vec_subMatX_u, C, alpha, delta, "exp")

  elseif model_type == "LuceLinearPRPF"
    # Prediction w.r.t user $u$ by luce-based list-wise LTR with linear transformation
    ret[1,js] = @fastmath user_preference_train_luce(vec_subPrior_X_u, vec_subPredict_X_u, vec_subMatX_u, C, alpha, delta, "linear")
